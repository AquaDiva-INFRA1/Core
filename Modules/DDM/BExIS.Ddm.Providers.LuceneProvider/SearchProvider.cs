...
namespace BExIS.Ddm.Providers.LuceneProvider
{
    ...
    public class SearchProvider : ISearchProvider
    {
        ...
        public SearchModel UpdateFacets(SearchCriteria searchCriteria = null, Occur ocuur = Occur.MUST)
        {
            if (searchCriteria == null)
                searchCriteria = new SearchCriteria();
            getQueryFromCriteria(searchCriteria, ocuur);
        ...
        public SearchModel UpdateProperties(SearchCriteria searchCriteria = null, Occur ocuur = Occur.MUST)
        {
            if (searchCriteria == null)
                searchCriteria = new SearchCriteria();
            getQueryFromCriteria(searchCriteria, ocuur);
        ...
        public SearchModel Get(SearchCriteria searchCriteria, Occur ocuur  )
        {
            getQueryFromCriteria(searchCriteria, ocuur);
            this.WorkingSearchModel.ResultComponent = BexisIndexSearcher.search(bexisSearching, SearchConfig.headerItemXmlNodeList);
        ...
        public void SearchAndUpdate(SearchCriteria searchCriteria, Occur ocuur = Occur.MUST)
        {
            this.WorkingSearchModel = Get(searchCriteria, ocuur);
            this.WorkingSearchModel = UpdateFacets(searchCriteria , ocuur);
            this.WorkingSearchModel = UpdateProperties(searchCriteria, ocuur);
        }
        ...
        /// <param name="searchCriteria"></param>
        /// <return></return>
        private void getQueryFromCriteria(SearchCriteria searchCriteria, Occur ocuur = Occur.MUST)
        {
                ...
                foreach (SearchCriterion sco in searchCriteria.SearchCriteriaList)
                {
                    if (sco.Values.Count > 0)
                    {
                        ...
                        else if (sco.SearchComponent.Type.Equals(SearchComponentBaseType.Facet))
                        {
                            ...
                            ((BooleanQuery)bexisSearching).Add(bexisSearchingFacet, ocuur);// and 
                        }
