...
namespace BExIS.Ddm.Providers.LuceneProvider
{
    public class SearchDesigner : ISearchDesigner
    {
        ...
        public List<Tuple<long,string>> GetEntitieNodes()
        {            if (_meanings.Count > 0)
                return _meanings.Select(meaning => Tuple.Create(meaning.Id, meaning.Name)).ToList();
            else            {                using (MeaningManager meaning_manager = new MeaningManager())
                {
                    _meanings = meaning_manager.getMeanings().Distinct().OrderBy(x => x.Name).ToList();
                }                return _meanings.Distinct().Select(entry => Tuple.Create(entry.Id, entry.Name)).ToList(); 
