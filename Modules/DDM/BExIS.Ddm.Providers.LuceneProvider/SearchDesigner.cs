...
using static BExIS.Aam.Entities.Mapping.Aam_Uri;
...
namespace BExIS.Ddm.Providers.LuceneProvider
{
    public class SearchDesigner : ISearchDesigner
    {
        ...
        private List<Aam_Uri> _entites = new List<Aam_Uri>();
        private List<Group> _projects = new List<Group>();
        ...
        private void Load()
        {
            ...
            foreach (XmlNode fieldProperty in fieldProperties)
            {
                if (!fieldProperty.Attributes.GetNamedItem("lucene_name").Value.Equals("Primarydata"))
                {
                    ...
                    if (fieldProperty.Attributes.GetNamedItem("entityIds") != null)
                        sa.entities = fieldProperty.Attributes.GetNamedItem("entityIds").Value;

                    if (fieldProperty.Attributes.GetNamedItem("projects") != null)
                        sa.projects = fieldProperty.Attributes.GetNamedItem("projects").Value;
        ...
        public List<Tuple<long,string>> GetEntitieNodes()
        {
            if (_entites.Count > 0)
                return _entites.Select(aamUri => Tuple.Create(aamUri.Id, aamUri.label)).ToList();
            else
            {
                using (Aam_Dataset_column_annotationManager annot_manag = new Aam_Dataset_column_annotationManager())
                {
                    _entites = annot_manag.get_all_dataset_column_annotation().Select(entry => entry.entity_id).Distinct().OrderBy(x => x.label.Trim()).ToList();
                    _entites.AddRange(annot_manag.get_all_dataset_column_annotation().Select(entry => entry.characteristic_id).Distinct().OrderBy(x => x.label.Trim()).ToList());
                }
                return _entites.Distinct().Select(aamUri => Tuple.Create(aamUri.Id, aamUri.Id.ToString()+" - "+aamUri.label)).ToList(); 
            }
        ...
        private XmlElement SetAttributesToNode(XmlElement xmlElement, SearchAttribute sa)
        {
            ...
            xa = this._configXML.CreateAttribute("entityIds");
            xa.Value = sa.entities;
            xmlElement.Attributes.Append(xa);

            xa = this._configXML.CreateAttribute("projects");
            xa.Value = sa.projects;
            xmlElement.Attributes.Append(xa);
        ...
        public List<Group> GetProjectsNodes()
        {
            ...
            else
            {
                ...
                {
                    _projects = grpManager.Groups.ToList();
                }
