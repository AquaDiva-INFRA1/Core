(function(a){try{if(document.execCommand){document.execCommand("BackgroundImageCache",false,true)}}catch(x){}
	var r=/\d/;var aC=/\s+/;var aB=parseInt(a.browser.version.substring(0,5).replace(".",""));
	var H=a.browser.mozilla&&aB>=180&&aB<=191;var s=/d{1,4}|M{1,4}|yy(?:yy)?|([Hhmstf])\1*|"[^"]*"|'[^']*'/g;
	...
	var aA=(function(){var e=new a.Event("triggerHandlerTest");a("<div />").triggerHandler(e);
		return !e.isDefaultPrevented()
	})();
	var b = a.telerik = {
		create: function (aF, aG)
		{
			var e = aG.name; var aE = a.extend({}, a.fn[e].defaults, aG.options);
			return aF.each(function(){var aH=a(this);aE=a.meta?a.extend({},aE,aH.data()):aE;if(!aH.data(e)){var aI=aG.init(this,aE);aH.data(e,aI);b.trigger(this,"load");
				if(aG.success){aG.success(aI)}}})},toJson:function(e){function aF(aG){return"["+a.map(aG,aE).join(",")+"]"}
					function aE(aH){var aI=[];for(var aG in aH){var aJ=aH[aG];if(a.isArray(aJ)){aI.push('"'+aG+'":'+aF(aJ))}else{
						if(typeof aJ!="object"){aI.push('"'+aG+'":"'+(aJ==null?"":aJ)+'"')}else{aI.push('"'+aG+'":'+aE(aJ))}}}
						return "{" + aI.join(",") + "}"
					} if (a.isArray(e))
					{
						return aF(e)
				...
				delegate: function (e, aE) { return function (aF) { aE.apply(e, [aF, this]) } },
				stop: function (aE, e)
				{
					return function (aF)
					{
